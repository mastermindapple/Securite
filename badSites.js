    ...
    .then((snapshot) => {
      ...
      if (childData.name !== undefined || childData.name !== null) {
        if (childData.badSites == undefined) {
            giveError("Your child has not visited any explicit sites.");
        } else {
          displaySites(childData.badSites,id);
        }
      } else {
...
function displaySites(sites,id) {
  ...
  for(site in sites){
    ...
    var badWordCount = document.createElement("td");
    badWordCount.innerHTML = sites[site].badWordCount;
